...
test("webpage-video HTTP adapter maps the dedicated API and concurrency evidence", async () => {
  ...
  const fetcher = async (url, init = {}) => {
    ...
    if (parsed.pathname === `/v1/webpage-video/runs/${runId}/site`) return json({
      ...
      storyboard: { status: "awaiting_review", revision: 4, sha256: "c".repeat(64), content: { shots: [{ id: "shot-1", page_id: "page-home", region_id: "hero", ordinal: 1, narration_cue: "核心标题", artifact: { preview_url: "https://cdn.example/hero.png" } }] } },
    });
  ...
  await adapter.reviewWebpageVideoStoryboard(runId, {
    decision: "approve", expectedRevision: 4, expectedSha256: "c".repeat(64), shots: [{ id: "shot-1", enabled: true, order: 1 }],
  ...
  assert.deepEqual(requests[6].body.shots, [{ id: "shot-1", enabled: true, order: 1 }]);
  assert.equal(requests[7].headers.get("Idempotency-Key"), "webpage-video-approve:fixed");
  assert.equal(requests[8].headers.get("Idempotency-Key"), "webpage-video-cancel:fixed");
});
...
test("webpage-video routes and components keep an isolated, fail-closed UI contract", async () => {
  const [createRoute, detailRoute, studio, detail, adapter, contracts, composer, css] = await Promise.all([
  ...
  for (const method of ["getWebpageVideoOptions", "createWebpageVideoRun", "getWebpageVideoRun", "getWebpageVideoCapture", "getWebpageVideoSite", "reviewWebpageVideoRun", "reviewWebpageVideoScope", "reviewWebpageVideoStoryboard", "cancelWebpageVideoRun"]) {
