...
Whisper 默认使用本机已缓存的 `small` 模型，默认不下载。新机器可显式设置 `FRAMEFACTORY_BENCHMARK_ALLOW_MODEL_DOWNLOAD=true` 做首次初始化；会联网下载模型，完成后建议关闭并固定本地模型路径。未缓存且未授权下载时会报告缺失或使用配置的云转写，不造时间戳。
...
- 全局单执行通道、最多 10 个待执行/活动任务；研究目录默认总量上限 2 GiB（预留当前任务工作空间）。空间不足拒绝，不删除其它文件腾空间。
...
- 回环浏览器获取用有界导航/滚动与下载期限；Python 采集线程不能强杀，超时后等待它退出才允许下一个任务，OS DNS 极端阻塞仍是残余风险。
- 新任务的源 MP4/临时片在流程结束后精确清理；任务目录、派生帧和音轨保留最多 7 天，在服务启动及周期检查时清理。SQLite 历史正文不会随媒体 TTL 删除；它计入目录预算，尚无归档删除/配额管理界面，长期使用需监控容量。停机期间不会运行清理。不要存放用户其它文件到这个专用目录。
